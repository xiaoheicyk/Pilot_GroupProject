...
// 确定员工的下一步操作和可执行的操作
const determineNextStepAndAction = (
  documents: Document[],
): { nextStep: string; action: "REVIEW" | "NOTIFY" | "NONE" } => {
...
// 确定员工的下一步操作和可执行的操作
const getCurrentReviewDocType = (
  documents: Document[],
): DocumentType | null => {
  // 按顺序检查文档状态
  if (documents[0].status === "PENDING" && documents[0].url)
    return "OPT_RECEIPT"
  if (
...
const VisaManagement = () => {
  ...
  const [activeTab, setActiveTab] = useState<"in-progress" | "all">(
    "in-progress",
  )
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        for (const emp of response.data) {
          try {
            ...
            try {
              ...
              console.error(
                `Error fetching OPT data for employee ${emp.id}:`,
                optErr,
              )
              ...
              if (employee.files && employee.files.length > 0) {
                console.log(
                  `Employee ${emp.id} has ${employee.files.length} files`,
                )

                // 查找OPT Receipt
                const optReceipt = employee.files.find(
                  (f: any) => f.name && f.name.includes("OPT Receipt"),
                )
                if (optReceipt) {
                ...
                // 查找OPT EAD
                const optEad = employee.files.find(
                  (f: any) => f.name && f.name.includes("OPT EAD"),
                )
                if (optEad) {
                ...
                // 查找I-983
                const i983 = employee.files.find(
                  (f: any) => f.name && f.name.includes("I-983"),
                )
                if (i983) {
                ...
                // 查找I-20
                const i20 = employee.files.find(
                  (f: any) => f.name && f.name.includes("I-20"),
                )
                if (i20) {
            ...
            employeesWithVisa.push({
              ...
              workAuthorization: {
                ...
                startDate:
                  employee.visaStart || employee.workAuth?.startDate || "",
        ...
        console.log(
          `Successfully processed ${employeesWithVisa.length} employees`,
        )
  ...
  const handleViewDocument = (employee: EmployeeWithVisa) => {
    // 找到当前需要审核的文档
    const pendingDocuments = employee.documents.filter(
  ...
  const handleDocumentAction = async (action: "APPROVED" | "REJECTED") => {
    ...
    try {
      ...
      console.log(
        `Sending ${backendAction} action for document ${backendDocType} of employee ${currentDocument.employee.id}`,
      )
      ...
      alert(
        `Failed to update document status: ${err.message || "Unknown error"}`,
      )
    }
  }
      ...
      <tr key={employee.id} className="border-b border-gray-200">
        ...
        <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="text-sm text-gray-500">
            {employee.workAuthorization.startDate} to{" "}
            {employee.workAuthorization.endDate}
          </div>
          <div
            className={`text-sm font-medium ${isExpired ? "text-red-600" : "text-blue-600"}`}
          >
            {remainingDays}
          </div>
        </td>
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                <h2 className="text-xl font-bold mb-4">
                  ...
                  {currentDocument.employee.firstName}{" "}
                  {currentDocument.employee.lastName}
                </h2>
